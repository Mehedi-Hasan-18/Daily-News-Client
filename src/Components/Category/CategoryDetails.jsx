...
const CategoryDetails = ({ index, category }) => {
  ...
  const handleExploreCategory = (categoryId) => {
    navigate(`/categories/${categoryId}/article`);
  };
    ...
    >
      <div className="p-6 flex flex-col h-full">
        ...
        <p className="text-gray-600 text-sm mb-4 flex-grow">
          {category.descriptions.slice(0,30)+"..."}
        </p>
        <button
          onClick={() => handleExploreCategory(category.id)}
          className="text-pink-500 font-bold hover:text-pink-600 transition-colors flex items-center"
        >
          Explore
          <FaAngleRight />
        </button>
